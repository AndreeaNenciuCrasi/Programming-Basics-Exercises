...
# of the sequence to generate the next. And, worse part of it, regrettably I won't get to hear non-native Italian speakers
...
def tribonacci(signature, n):
    if n == 0:
        return []
    elif n == 1 and signature[0] == signature[1] == signature[2]:
        return [signature[0]]
    tribonacci_list = signature
    i = 0
    while i < n - 3:
        tribonacci_list.append(
            tribonacci_list[i] + tribonacci_list[i + 1] + tribonacci_list[i + 2])
        i += 1
